#
# Created by Meng Lv on 2025/8/9.
#
...
class RootManager:
    ...
    def __init__(self, filename, runN, sample_num, chunk_size=1000):
        self._runN = runN
        self._file = uproot.recreate(filename)
        self.filename = filename
        print("DAQ Starts. Open ROOT file:", self.filename)
        if (sample_num>0):
            self._branch = {
                "Run": "int32", 
                "WaveN": "int32",
                ...
                "ChA": "{} * float32".format(sample_num), 
                "ChB": "{} * float32".format(sample_num), 
                "ChC": "{} * float32".format(sample_num), 
                "ChD": "{} * float32".format(sample_num)}
    ...
    def fill(self, **wave):
        ...
        required_key = {"Time", "ChA", "ChB", "ChC", "ChD"}
    ...
    def close(self):
        ...
        self._file.close()
        print(f"ROOT closed. {self._tree.num_entries} entries saved to: ", self.filename)

def fastAdc2mV(bufferADC, range, maxADC):
    """ 
        adc2mc(
    ...
    bufferV = bufferADC.astype('int64') * vRange / maxADC.value
    
    return bufferV

class RingBuffer:
    def __init__(self, ring_size, dtype='float32'):
        self._buffer = np.zeros(ring_size, dtype=dtype)
        self._size = int(ring_size)
        self._dtype = dtype
        self._widx = 0  # write index

    def ReadSlice(self, start_idx, length):
        if length > self._size:
            print('RingBuffer::ReadSlice: length larger than buffer size!')
            return
        if length <= 0:
            print('RingBuffer::ReadSlice: non-positive length!')
            return
        true_start_idx = start_idx % self._size
        true_end_idx = (start_idx+length) % self._size

        if true_end_idx > true_start_idx:
            return self._buffer[true_start_idx:true_end_idx].copy()
        else:
            return np.concatenate((self._buffer[true_start_idx:], self._buffer[:true_end_idx])).copy()

    def Push(self, arr):
        length = arr.size
        if length > self._size:
            print(f'RingBuffer::WriteSlice: length {length} larger than buffer size {self._size}!')
            return
        tail_space = self._size - self._widx
        if length <= tail_space:
            self._buffer[self._widx:self._widx+length] = arr[:]
        else:
            n_head = length - tail_space
            self._buffer[self._widx:] = arr[:tail_space]
            self._buffer[:n_head] = arr[tail_space:]
        self._widx = (self._widx + length) % self._size
